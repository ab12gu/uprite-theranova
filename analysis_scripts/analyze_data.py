...
def extract(directory):
    ...
    for k, p in enumerate(pace):
        ...
        angular_pos['z'] = integrate.IMU(gyro['sec'], gyro_hpf['z'],
                                         units='rad')
        ax.plot(accel['sec'], angular_pos['z'], color='g',
                label='tailbone z gyro')
        ...
        for o in orientation:
            for dot, f in enumerate(foot):
                ...
                for i in range(0, len(zeno['S'][f][o])):
                    if i == 0:
                        ax.axvline(x=zeno['S'][f][o][i] + offset,
                                   color=spectrum[counting], label=f + ' ' + o,
                                   linewidth=1, linestyle=linestyle[counting],
                                   dashes=(5, 10 * dot))
                    else:
                        ax.axvline(x=zeno['S'][f][o][i] + offset,
                                   color=spectrum[counting], linewidth=1,
                                   linestyle=linestyle[counting],
                                   dashes=(5, 10 * dot))
                counting += 1

        for i in range(0, len(stomp)):
            ax.axvline(x=stomp[i] + offset, color=spectrum[4],
                       label='Stomp ' + str(i), linewidth=1)
