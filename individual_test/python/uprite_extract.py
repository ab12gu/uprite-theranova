...
from individual_test.python_utils import stride
from individual_test.python_utils import difference
from individual_test.python_utils import find_peaks
from individual_test.python_utils import filt
import individual_test.python_utils.integrate_IMU as integrate

start_time = clocktime.time()
pickle_file = '../test_data/no_001.pkl'
...
yaw = math.atan(
    normalized_mean_accel['y'] / math.sqrt(normalized_mean_accel['x'] ** 2 + \
...
gravity_accel = {}
gravity_accel['x'], _, _, _ = filt.lowpass(accel_data['x'], f_cuts,
                                           sampling_rate, \
                                           ripple_tol)
gravity_accel['y'], _, _, _ = filt.lowpass(accel_data['y'], f_cuts,
                                           sampling_rate, \
                                           ripple_tol)
gravity_accel['z'], _, _, _ = filt.lowpass(accel_data['z'], f_cuts,
                                           sampling_rate, \
                                           ripple_tol)
...
for i in range(0, len(accel['sec'])):
    ...
    gravity_vector_holder[:, i] = rotation['y'] @ rotation['z'] @ rotation[
        'x'] @ g_v
    ...
    a = inv(
        updated_rotation['y'] @ updated_rotation['z'] @ updated_rotation['x'])

    earth2_accel_delta_vector_holder[:, i] = a @ accel2_delta_vector_holder[:,
                                                 i]
    ...
    updated_rotation['z'][0] = [math.cos(new_ang['z']), math.sin(new_ang['z']),
                                0]
    ...
    if (i > 1):
        ...
        roll = math.atan(
            normalized_mean_position['y'] / normalized_mean_position['z'])
        pitch = math.atan(-1 * normalized_mean_position['x'] / math.sqrt( \
            normalized_mean_position['y'] ** 2 + normalized_mean_position[
                'z'] ** 2))
        ...
        a = inv(
            updated_rotation['x'] @ updated_rotation['y'] @ updated_rotation[
                'z'])
...
pure_delta_velocity, pure_delta_position = integrate.double(accel['sec'],
                                                            accel_delta)
...
for i in range(0, 5):
    stride_velocity[i]['r'], mean_stride_velocity[i]['r'] = stride.vel(time, \
                                                                       ...
                                                                       HS[i][
                                                                           'r'])
    stride_velocity[i]['l'], mean_stride_velocity[i]['r'] = stride.vel(time, \
                                                                       earth_position[
                                                                           'z'],
                                                                       earth_velocity[
                                                                           'z'],
...
f_cuts = [10 / Ny, 11 / Ny]
earth_filt = {}
earth_filt['x'], _, _, _ = filt.lowpass(earth_accel_delta_vector_holder[0, :], \
...
cut = 0.5
gyro2_hpf = {}
gyro2_hpf['z'], _, _, _ = filt.highpass(gyro['z'], fs, cut)
angular_pos[1] = {}
...
with open('../docs/python_data.csv', 'w') as csvfile:
    filename = csv.writer(csvfile)
    filename.writerow(top)
