...
def gyro_peaks(angular_pos, gyro):
    ...
    backward_peaks = find_peaks.forward(temp, search_size, min_dist, max_dist,
                                        fs)
...
def gyro_troughs(angular_pos, gyro):
    ...
    # forward troughs
    troughs = find_peaks.forward(neg_ang_pos, search_size, min_dist, max_dist,
                                 fs)

    # backward troughs
    offset = list(reversed(neg_ang_pos))
    backward_troughs = find_peaks.forward(offset, search_size, min_dist,
                                          max_dist, fs)
