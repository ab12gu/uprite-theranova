...
def right_left(accel_peaks, all_gyro_spikes, gyro_troughs, to_locations):
    ...
    for i in range(0, len(gyro_troughs)):  # iterate through all troughs
        ...
        if (i != len(gyro_troughs) - 1):  # make sure i isn't last element
            ...
            gyro_peak = [x for x in all_gyro_spikes if
            ...
            if len(gyro_peak) > 1:  # more than single peak
                ...
                accel_peak = [x for x in accel_peaks if
                              first_trough < x < gyro_peak]

                if (accel_peaks and len(
                        accel_peak) == 1):  # if only 1 peak and there is acceleration peaks
                    accel_peak = accel_peak[0]  # remove out of list

                    HS['r'].append(
                        accel_peak)  # right heel is after first gyro trough

                    to_location = [x for x in to_locations if
                                   first_trough < x < gyro_peak]  # find closest toe off (in same window)
                    [TO['l'].append(x) for x in
                     to_location]  # save all toes offs in window
            else:  # only 1 peak
            ...
            if (accel_peaks and len(
                ...
                to_location = [x for x in to_locations if
                               first_trough < x < gyro_peak]  # find closest toe off (in same window)
                [TO['l'].append(x) for x in
                 to_location]  # save all toe offs in window

            # other leg movement
            accel_peak = [x for x in accel_peaks if
                          gyro_peak < x < second_trough]
