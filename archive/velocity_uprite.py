...
def extract(directory):
    ...
    if data_wdw['F'][1] > len(gyro_all['x']):
    ...
    for p in pace:
        ...
        gravity_strength = math.sqrt(
            mean_accel['x'] ** 2 + mean_accel['y'] ** 2 + \
            mean_accel['z'] ** 2)
        ...
        """NU"""
        normalized_gravity_vector = [normalized_mean_accel['x'],
                                     normalized_mean_accel['y'], \
        ...
        pitch = math.atan(
            normalized_mean_accel['z'] / normalized_mean_accel['x'])
        yaw = math.atan(normalized_mean_accel['y'] / math.sqrt(
            normalized_mean_accel['x'] ** 2 + \
        ...
        for i in range(0, len(accel['sec'])):
            ...
            rotation['z'][0] = \
                [math.cos(angular_pos['z'][i]), math.sin(angular_pos['z'][i]),
                 0]
            rotation['z'][1] = \
                [-1 * math.sin(angular_pos['z'][i]),
                 math.cos(angular_pos['z'][i]), 0]
            rotation['z'][2] = [0, 0, 1]
            ...
            gravity_vector_holder[:, i] = rotation['y'] @ rotation['z'] @ \
                                          rotation['x'] @ g_v
            ...
            accel2_delta_vector_holder[:, i] = \
                [accel2_delta['x'][i], accel2_delta['y'][i],
                 accel2_delta['z'][i]]
            ...
            updated_rotation['x'][1] = \
                [0, math.cos(angular_pos['x'][i]),
                 math.sin(angular_pos['x'][i])]
            updated_rotation['x'][2] = \
                [0, -1 * math.sin(angular_pos['x'][i]),
                 math.cos(angular_pos['x'][i])]

            updated_rotation['y'] = np.empty([3, 3])
            updated_rotation['y'][0] = [math.cos(angular_pos['y'][i] + pitch),
                                        0, \
                                        -1 * math.sin(
                                            angular_pos['y'][i] + pitch)]
            updated_rotation['y'][1] = [0, 1, 0]
            updated_rotation['y'][2] = [math.sin(angular_pos['y'][i] + pitch),
                                        0, \
            ...
            updated_rotation['z'][1] = [
                -1 * math.sin(angular_pos['z'][i] + yaw), \
            ...
            i] = a @ accel2_delta_vector_holder[:, i]
            ...
            updated_normal_mean_accel['x'] = gravity_accel['x'][
                                                 i] / updated_gravity_strength
            updated_normal_mean_accel['y'] = gravity_accel['y'][
                                                 i] / updated_gravity_strength
            updated_normal_mean_accel['z'] = gravity_accel['z'][
                                                 i] / updated_gravity_strength
            ...
            new_ang['z'] = math.atan(updated_normal_mean_accel['y'] / math.sqrt( \
                updated_normal_mean_accel['x'] ** 2 + updated_normal_mean_accel[
                    'z'] ** 2))
            # updated yaw
            ...
            updated_rotation['y'][0] = [math.cos(new_ang['y']), 0,
                                        -1 * math.sin(new_ang['y'])]
            updated_rotation['y'][1] = [0, 1, 0]
            updated_rotation['y'][2] = [math.sin(new_ang['y']), 0,
                                        math.cos(new_ang['y'])]

            updated_rotation['z'][0] = [math.cos(new_ang['z']),
                                        math.sin(new_ang['z']), 0]
            updated_rotation['z'][1] = [-1 * math.sin(new_ang['z']),
            ...
            i] = a @ accel_delta_vector_holder[:, i]
            ...
            if (i > 1):
                ...
                position_strength = math.sqrt(
                    earth_position_temp['x'][-1] ** 2 + \
                    earth_position_temp['y'][-1] ** 2 +
                    earth_position_temp['z'][-1] ** 2)

                # Normalize the distance moved? I don't see the point of this...
                normalized_mean_position = {}
                normalized_mean_position['x'] = earth_position_temp['x'][
                                                    -1] / position_strength
                normalized_mean_position['y'] = earth_position_temp['y'][
                                                    -1] / position_strength
                normalized_mean_position['z'] = earth_position_temp['z'][
                                                    -1] / position_strength

                # Where did this come from?
                roll = math.atan(
                    normalized_mean_position['y'] / normalized_mean_position[
                        'z'])
                pitch = math.atan(
                    -1 * normalized_mean_position['x'] / math.sqrt( \
                        normalized_mean_position['y'] ** 2 +
                        normalized_mean_position['z'] ** 2))

                updated_rotation['x'][0] = [1, 0, 0]
                updated_rotation['x'][1] = [0, math.cos(roll), math.sin(roll)]
                updated_rotation['x'][2] = [0, -1 * math.sin(roll),
                                            math.cos(roll)]

                updated_rotation['y'][0] = [math.cos(pitch), 0,
                                            -1 * math.sin(pitch)]
                updated_rotation['y'][1] = [0, 1, 0]
                ...
                forward_earth_accel_delta_vector_holder[:, i] = a @ \
                                                                earth_accel_delta_vector_holder[
                                                                :, i]
                forward_position_delta_vector_holder[:, i] = a @ [
                    earth_position_temp['x'][-1], \
        ...
        earth_velocity, earth_position = integrate.double(accel['sec'],
                                                          earth_accel_delta_hpf)
        ...
        # get velocity
        forward_earth_velocity = integrate.single(accel['sec'],
                                                  forward_earth_accel_delta_hpf)
        # multiply it by 4? wtf
        forward_earth_velocity['z'] = [4 * x for x in
        ...
        for c, axis in enumerate(coordinates):
            plt.figure()
            plt.plot(time, [9.81 * x for x in
                            forward_earth_accel_delta_vector_holder[c, :]])
            plt.plot(time,
        ...
        peaks, _, _, _ = find_peaks.forward(pos_ang_pos['z'], search_size,
                                            min_dist, \
                                            max_dist, fs)
        temp = list(reversed(pos_ang_pos['z']))
        backward_peaks, _, _, _ = find_peaks.forward(temp, search_size,
                                                     min_dist, \
                                                     max_dist, fs)
        ...
        accel_peaks, _, _, _ = find_peaks.forward(earth_filt['x'], search_size,
                                                  min_dist, \
                                                  max_dist, fs)
        ...
        [plt.axvline(gyro['sec'][x], color='g', linestyle='--') for x in
         all_peaks]
        [plt.axvline(gyro['sec'][x], color='b', linestyle='--') for x in
         all_troughs]

        # figure 14b
        plt.subplot(212)
        plt.plot(gyro['sec'], earth_filt['x'])
        plt.plot([accel['sec'][x] for x in all_accel_peaks],
                 [earth_filt['x'][x] for x \
                  in all_accel_peaks], 'gx')
        plt.plot([accel['sec'][x] for x in to_locations],
                 [earth_filt['x'][x] for x in \
                  to_locations], 'k^')
        ...
        for i in range(0, len(all_troughs)):
            if (i != len(all_troughs) - 1):
                ...
                peak = [x for x in all_peaks if
                        first_trough < x < second_trough]
                if not peak:
                    continue
                elif len(peak) > 1:
                    peak = peak[
                ...
                # left leg movement
                accel_peak = [x for x in all_accel_peaks if
                              peak < x < second_trough]

                if (not not accel_peak and len(accel_peak) == 1):
                    accel_peak = accel_peak[0]
                    HS['l'].append(accel_peak)
                    to_location = [x for x in to_locations if
                                   peak < x < second_trough]
    ...
    if (accel_flag == 0):
        print('No data recorded in patient:', patient_name)
        return
    ...
    while (check == 0):
        ...
        window['flag'] = {}
        window['flag']['auto'] = int(
            input("Enter 0 if no auto data (else 1): "))
