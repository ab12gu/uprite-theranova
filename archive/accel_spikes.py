...
def accel_spikes(accel, gyro_peaks):
    ...
    for i in range(0, len(accel['sec'])):
        ...
        accel_delta_vector[:, i] = [accel_delta['x'][i], accel_delta['y'][i],
                                    accel_delta['z'][i]]
        ...
        grav_ang = {}
        grav_ang['y'] = math.atan(
            norm_grav['z'] / norm_grav['x'])  # updated pitch
        grav_ang['z'] = math.atan(norm_grav['y'] / math.sqrt(
            norm_grav['x'] ** 2 + norm_grav['z'] ** 2))

        rotation['x'] = np.identity(3)

        rotation['y'][0] = [math.cos(grav_ang['y']), 0,
                            -1 * math.sin(grav_ang['y'])]
        rotation['y'][1] = [0, 1, 0]
        rotation['y'][2] = [math.sin(grav_ang['y']), 0, math.cos(grav_ang['y'])]

        rotation['z'][0] = [math.cos(grav_ang['z']), math.sin(grav_ang['z']), 0]
        rotation['z'][1] = [-1 * math.sin(grav_ang['z']),
    ...
    for i in range(0, len(gyro_peaks) - 1):
        ...
        accel_peak = find_peaks.forward(
            earth_filt['x'][start_index:end_index + 1],
