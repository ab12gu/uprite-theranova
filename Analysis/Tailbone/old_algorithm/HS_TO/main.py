...
output_directory = root + '/AG_parsed_Data/UR/test'
...
for c, filename in enumerate(os.listdir(data_input_dir)):
	...
	for p in pace:
		...
		gravity_accel = {}
		gravity_accel['x'] = filt.lowpass(accel_data['x'], f_cuts, sampling_rate, \
				ripple_tol)
		gravity_accel['y'] = filt.lowpass(accel_data['y'], f_cuts, sampling_rate, \
				ripple_tol)
		gravity_accel['z'] = filt.lowpass(accel_data['z'], f_cuts, sampling_rate, \
				ripple_tol)
		...
		f_cuts = [10/Ny, 11/Ny]
		earth_filt = {}
		earth_filt['x'] = filt.lowpass(earth_accel_delta_vector_holder[0,:], \
		...
		cut = 0.5
		gyro2_hpf = {}
		gyro2_hpf['z'] = filt.highpass(gyro['z'], fs, cut)
		angular_pos[1] = {}
		...
		cut = [5/Ny,6/Ny]
		angular_pos[2] = {}
		angular_pos[2]['z'] = filt.lowpass(angular_pos[1]['z'], cut, fs, ripple_tol)
	...
	print("Completed HS & TO for patient: ", filename)
	plt.show()
	continue
