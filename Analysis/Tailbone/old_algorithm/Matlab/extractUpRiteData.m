...
fid = fopen(fileName);
firstLine = fgets(fid); %get the header information
...
firstLineInfo = strsplit(firstLine,',\t');
for i = 1:length(firstLineInfo)
    data = strsplit(firstLineInfo{i},': ');
    if strcmp('sampRate',data{1})
